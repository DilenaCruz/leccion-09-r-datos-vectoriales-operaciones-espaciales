...
cr_reptilia <- 
  ...
  ) %>% # estos son los  comandos para delimitar o transformar el sistema de coordenadas de las capas
  st_set_crs(4326) %>% 
  st_transform(5367)
```
...
plot(cr_aerodromos$geometry, col="blue", pch=16, add=TRUE)
```
...
plot(aerodromos_limon$geometry, col="blue", pch=16, add=TRUE)
```
...
# Mapeo
plot(limon$geometry, axes=TRUE, graticule=TRUE, reset=FALSE)
plot(asp_limon$geometry, col="darkgreen", add=TRUE)
...
# el parámetro intersects muestra tanto los polígonos que están parcialmente contenidos como los que contiene en su totalidad
...
#Usando overlaps, incluye los polígonos que está parcialmente contenidos

# ASP que se intersecan con la provincia de Limón

# Geometrías (polígonos) de ASP que se intersecan en la geometría de Limón
asp_limon <- cr_asp[limon, , op = st_overlaps]
...
alajuela_heredia <-
  cr_provincias %>%
  filter(provincia == "Alajuela" | provincia == "Heredia")


cr_colubridae <-
  cr_reptilia %>%
  filter(family == "Colubridae")

alaj_herd_colubridae <- cr_colubridae[alajuela_heredia, , op = st_within]

plot(alajuela_heredia$geometry, axes=TRUE, graticule=TRUE, reset=FALSE)
plot(alaj_herd_colubridae$geometry, col="brown", pch=4, add=TRUE)


alaj_herd_colubridae  %>%
  st_drop_geometry() %>%
  distinct(alaj_herd_colubridae$species)
```
...
m <- leaflet() %>% 
  ...
  addCircleMarkers(data = guanacaste_viperidae_wgs84,
                   ...
                   popup = paste(guanacaste_viperidae_wgs84$genus, 
                                 ...
                                 guanacaste_viperidae_wgs84$eventDate, 
                                 sep = '<br/>')
                   ) %>%
...
3. Repita el ejercicio de agregar los datos de los sitios altos de Nueva Zelanda, pero de acuerdo con el valor máximo.
